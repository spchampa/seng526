 ...
 * Assignment:  1
 * 
 * Modified: 2017-09-24 by Steven Champagne
 * File Name: modifiedSecretServer.c
 *********************************************/
...
void readLineFromFd( int fd, char * buff) {
    char * ptr = buff;
    while((ptr - buff) < sizeof(globals.buffer)-1) // Just changed this from while(1) to fix pointer boundary.
	{
...
int main( int argc, char ** argv)
{
    ...
    while(1) {
		...
		printf( "The current password is:  %s\n", globals.password); // I added this printf
        ...
        writeStrToFd( connSockFd, "Secret Server 2.0\n");
